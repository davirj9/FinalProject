	...
	$.alerts = {
		...
		_show: function(title, msg, value, type, callback) {
			...
			switch( type ) {
				case 'success':
					...
					$("#popup_ok").click( function() {
						if($(this).attr('name') == 'redirect'){
							window.location.href = $("#contextPath").val() + $(this).attr('title');
						}else
							if($(this).attr('name') == 'close'){
								window.close();
							}else
								if($(this).attr('name') == 'refresh'){
									window.location.reload(); 
								}else
									if($(this).attr('name') == 'redirectAguardando'){
										$("#aguardando").show();
										window.location.href = $("#contextPath").val() + $(this).attr('title');
									}
						$.alerts._hide();
				...
				break;
				case 'confirm':
					$("#popup_message").after('<div id="popup_panel"><input type="button" class="btn btn-default" value="' + $.alerts.yesButton + '" id="popup_ok" /> <input type="button" class="btn btn-default" value="' + $.alerts.noButton + '" id="popup_cancel" /></div>');
					$("#popup_ok").click( function() {
						$.alerts._hide();
		...
		_reposition: function() {
			...
			var left = (($(window).width() / 2) - (($("#popup_container").outerWidth() / 2) + 40)) + $.alerts.horizontalOffset;
			if( top < 0 ) top = 0;
			if( left < 0 ) left = 0;
